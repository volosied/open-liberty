...
public class CDISessionScopedBean implements Serializable {
    ...
    @PostConstruct
    public void start() {
        System.out.println("CDISessionScopedBean postConstruct called");
        this.data += ":PostConstructCalled:";
        if (ejb != null && ejb.verifyPostConstruct())
            this.data += ":EJB-injected:";
        if (defaultExec != null)
            this.data += ":Resource-injected:";
        System.out.println("CDISessionScopedBean data is: " + data);
    }

    @PreDestroy
    public void stop() {
        System.out.println("CDISessionScopedBean preDestroy called.");
    }
