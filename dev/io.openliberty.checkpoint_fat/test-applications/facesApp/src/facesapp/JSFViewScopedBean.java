...
@ManagedBean(name="jsfViewScopedBean", eager=true)
@ViewScoped
public class JSFViewScopedBean implements Serializable {
    ...
    @PostConstruct
    public void start() {
        System.out.println("jsfViewScopedBean postConstruct called");
        this.data += ":PostConstructCalled:";
        if (ejb != null && ejb.verifyPostConstruct())
            this.data += ":EJB-injected:";
        System.out.println("jsfViewScopedBean data is: " + data);
    }

    @PreDestroy
    public void stop() {
        System.out.println("jsfViewScopedBean preDestroy called.");
    }
