...
public class TldParser extends DefaultHandler {
    ...
    public TldParser(JspCoreContext ctxt,
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(logLevel)) {
            logger.logp(logLevel, CLASS_NAME, "TldParser(JspCoreContext, JspConfigurationManager, boolean, ClassLoader)",
                        "ctxt= [" + ctxt + "]  configManager= [" + configManager + "] validateTLDs= [" + validateTLDs + "] classloader= [" + classloader + "]");
        }
    }
    ...
    public TldParser(JspCoreContext ctxt,
        ...
        try {
            ...
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(logLevel)) {
                logger.logp(logLevel, CLASS_NAME, "TldParser(JspCoreContext, JspConfigurationManager, boolean)",
                            "ctxt= [" + ctxt + "]  configManager= [" + configManager + "] validateTLDs= [" + validateTLDs + "] saxParser= [" + saxParser + "]");
            }
    ...
    public TagLibraryInfoImpl parseTLD(JspInputSource inputSource, InputStream is, String tldOriginatorId) throws JspCoreException {
        ...
        tli = new TagLibraryInfoImpl(tldOriginatorId, inputSource);
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(logLevel)) {
            logger.logp(logLevel, CLASS_NAME, "parseTLD(JspInputSource, InputStream, String)",
                        "inputSource= [" + inputSource + "]  InputStream=[" + is + "] tldOriginatorId= [" + tldOriginatorId + "]");
        }
        try {
            parse(is);
        } catch (SAXException e) {
    ...
    public void endElement(String namespaceURI,
        ...
        if (elementName.equals("tlibversion") || elementName.equals("tlib-version")) {
            ...
            TagVariableInfo tvi = new TagVariableInfo(nameGiven, nameFromAttribute, variableClassName, declare, scope);
            variables.add(tvi);
            resetVariable();
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(logLevel)) {
            ...
            TagAttributeInfo tai = new TagAttributeInfo(attributeName, required, type, reqTime, fragment, null, deferredValue, deferredMethod, expectedType, methodSignature); // jsp2.1ELWork
            attributes.add(tai);
            resetAttribute();
            currentElement = TAG_ELEMENT;
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(logLevel)) {
            ...
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(logLevel)) {
                logger.logp(logLevel, CLASS_NAME, "endElement",
                            "elementName= [" + elementName + "] deferredMethod= [" + deferredMethod + "] methodSignature= [" + methodSignature + "]");
            }
