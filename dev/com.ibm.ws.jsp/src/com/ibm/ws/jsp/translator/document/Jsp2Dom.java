...
public class Jsp2Dom {
    ...
    private void getSyntaxAndEncoding() throws JspCoreException {
        ...
        try {
            ...
            if (isBomPresent) {
                ...
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                    logger.logp(Level.FINER, CLASS_NAME, "getSyntaxAndEncoding", "BOM is present, attempt to use page encoding from BOM, sourceEnc = {0}",
                                new Object[] { sourceEnc });
                }
                ...
                if (sourceEncFromConfig != null) {
                    ...
                    if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                        logger.logp(Level.FINER, CLASS_NAME, "getSyntaxAndEncoding", "Attempt to use page encoding from page directives, sourceEnc = {0}",
                                    new Object[] { sourceEnc });
                    }
    ...
    private Document getJspDocumentAsJspPage() throws JspCoreException {
        ...
        try {
            ...
            JspPageParser jspPageParser = new JspPageParser(br, jspInputSource, resolvedRelativeURL, context, directoryStack, jspConfiguration, jspOptions, //396002
                            dependencyStack, dependencyList, cdataJspIdMap, implicitTagLibMap);
            try {
    ...
    protected Document parseToDom() throws JspCoreException {
        ...
        try {
            ...
            JspDocumentParser jspDocumentParser = new JspDocumentParser(jspInputSource, resolvedRelativeURL, context, jspConfiguration, jspOptions, // 396002
                            directoryStack, dependencyStack, dependencyList, cdataJspIdMap, implicitTagLibMap, isBomPresent, isEncodingSpecifiedInProlog, sourceEnc);
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
    ...
    private void detectXMLEncoding() throws JspCoreException {
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
            ...
            logger.logp(Level.FINER, CLASS_NAME, "Jsp2Dom",
                        "detectXMLEncoding back from XMLEncodingDetector  isEncodingSpecifiedInProlog: [" + this.isEncodingSpecifiedInProlog + "]");
