...
public abstract class ValidateVisitor extends JspVisitor {
    ...
    private static final String[] pluginAttrNames = { "type", "code", "codebase", "align", "archive", "height", "hspace", "jreversion", "name", "vspace", "title", "width",
                                                      "nspluginurl", "iepluginurl", "mayscript" };
    ...
    protected void visitCustomTagStart(Element jspElement) throws JspCoreException {
        ...
        if (tei != null) {
            ...
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                logger.logp(Level.FINER, CLASS_NAME, "visitCustomTagStart",
                            "tei.getTagInfo().getTagLibrary().getTagLibraryInfos() =[" + tei.getTagInfo().getTagLibrary().getTagLibraryInfos() + "]");
            }
        ...
        if (classAttrValue.equals("") == false && typeAttrValue.equals("") == false) {
            try {
                ...
                if (typeAttrClass.isAssignableFrom(classAttrClass) == false) {
                    throw new JspTranslationException(jspElement, "jsp.error.usebean.class.must.be.assignable.to.type", new Object[] { classAttrValue, typeAttrValue,
                                                                                                                                       jspElement.getTagName() });

                }
            } catch (ClassNotFoundException cnfe) {
                throw new JspTranslationException(jspElement, "jsp.error.usebean.cannot.locate.class.to.validate.assignable", new Object[] { classAttrValue, typeAttrValue,
                                                                                                                                             jspElement.getTagName() }, cnfe);
            }
        }
    ...
    protected void visitJspOutputStart(Element jspElement) throws JspCoreException {
        ...
        if (jspOptions != null) {
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                logger.logp(Level.FINER, CLASS_NAME, "visitJspOutputStart",
                            "jspOptions.isAllowJspOutputElementMismatch():  [" + jspOptions.isAllowJspOutputElementMismatch() + "]");
            }
            ...
            for (int j = 0; j < childNodes.getLength(); j++) {
                ...
                if (n.getNodeType() == Node.CDATA_SECTION_NODE) {
                    ...
                    if (value.trim().length() > 0) {
                        throw new JspTranslationException(jspElement, "jsp.error.jspbody.emptybody.only", new Object[] { jspElement.getNodeName(),
                                                                                                                         "[" + n.getNodeName() + "] : " + value });
                    }
    ...
    protected CustomTagInstance validateCustomTagAttributeValues(Element jspElement, TagAttributeInfo[] attributes, boolean hasDynamicAttributes,
                                                                 TagLibraryInfoImpl tli) throws JspCoreException {
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
        ...
        for (int i = 0; i < nnmTop.getLength(); i++) {
            ...
            if (nodeName.equals("jsp:id") == false && nodeName.startsWith("xmlns") == false) {
                ...
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                    logger.logp(Level.FINER, CLASS_NAME, "validateCustomTagAttributeValues",
                                "attributes [" + attributes + "] nodeName [" + nodeName + "]  nodeValue [" + nodeValue + "]");
                ...
                //else {
                isScriptless = validateCustomTagAttribute(jspElement, attributeMap, isScriptless, checkDeferred, nodeName, tai, nodeValue, dynamicDeferredMethod,
                                                          dynamicDeferredMethodSignature);
                //}
            }
        }
        NodeList nl = jspElement.getChildNodes();
            ...
            if (n.getNodeType() == Node.ELEMENT_NODE) {
                ...
                if (e.getNamespaceURI() != null && e.getNamespaceURI().equals(Constants.JSP_NAMESPACE) && e.getLocalName().equals(Constants.JSP_ATTRIBUTE_TYPE)) {
                    ...
                    if (tai == null) {
                         ...
                         * if(com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable(Level.FINER)){
                         * logger.logp(Level.FINER, CLASS_NAME, "validateCustomTagAttributeValues","deferredMethodSignature is null ... setting to " +
                         * dynamicDeferredMethodSignature);
                         * }
                         * } else {
                         * if(com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled()&&logger.isLoggable(Level.FINER)){
                            ...
                            // The expression is only one child
                                ...
                                if (!areChildrenScriptless(e)) {
                                    ...
                                    if (tai != null && tai.canBeRequestTime() == false) {
                                        throw new JspTranslationException(jspElement, "jsp.error.attribute.cannot.be.request.time", new Object[] { nodeName,
                                                                                                                                                   childAttrNode.getNodeValue() });
                                    } else {
                                        isScriptless = false;
    ...
    private boolean validateCustomTagAttribute(Element jspElement, Hashtable attributeMap, boolean isScriptless, boolean checkDeferred, String nodeName, TagAttributeInfo tai,
                                               String nodeValue, boolean dynamicDeferredMethod,
                                               String dynamicDeferredMethodSignature) throws JspTranslationException, JspCoreException {
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
            ...
            if (tai != null) {
                logger.logp(Level.FINER, CLASS_NAME, "validateCustomTagAttribute", "tai.canBeRequestTime() =[" + tai.canBeRequestTime() + "] tai.isDeferredMethod()= ["
                                                                                   + tai.isDeferredMethod() + "] tai.isDeferredValue()= [" + tai.isDeferredValue() + "]");
            } else {
    ...
    private final static void ELCheckType(final Object obj, final Class<?> type) throws ELException {
        ...
        if (Boolean.class.equals(type) || Boolean.TYPE == type) {
            ELSupport.coerceToBoolean(null, obj, true);
        }
        if (type.isEnum()) {
