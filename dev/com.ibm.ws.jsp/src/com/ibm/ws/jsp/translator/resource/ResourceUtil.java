...
public class ResourceUtil {
    ...
    public static void sync(long sourceFileTimeStamp, File generatedSourceFile, File classFile, String className, boolean keepgenerated, boolean keepGeneratedclassfiles) {
        ...
        if (keepGeneratedclassfiles == false) {
            ...
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
                logger.logp(Level.FINEST, CLASS_NAME, "sync",
                            (rc ? "Updated" : "Unable to update") + " lastModified timestamp for classFile [" + classFile + "] [" + classFile.lastModified() + "]");
            }
        }
        ...
        if (generatedSourceFile.exists()) {
            if (keepgenerated == false) {
                ...
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
                    logger.logp(Level.FINEST, CLASS_NAME, "sync", (rc ? "Updated" : "Unable to update") + " lastModified timestamp for generatedSourceFile [" + generatedSourceFile
                                                                  + "] [" + generatedSourceFile.lastModified() + "]");
                }
            }
        }
    }
    ...
    public static boolean isOutdated(long lastModified, Entry containerEntry, File sourceFile, File generatedSourceFile, File classFile, Entry webinfClassEntry,
                                     File webinfClassFile) {
        ...
        if (sourceFile != null || isContainerOrEDR) {
                ...
                if (classFile.exists() && jspCaseCheck(classFile, classFile.getAbsolutePath())) {
                    if (sourceLastModified == classFile.lastModified()) {
                        ...
                        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                            ...
                            logger.logp(Level.FINER, CLASS_NAME, "isOutdated",
                                        "sourceFile ts [" + sourceLastModified + "] differs from tempDirClassFile ts [" + classFile.lastModified() + "]. Recompile JSP.");
                        }
                    }
                    // PK75069 - call private jspCaseCheck() instead of FileSytem.uriCaseCheck()
                    ...
                    if (sourceLastModified == webinfLastModified) {
                        ...
                        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                            ...
                            logger.logp(Level.FINEST, CLASS_NAME, "isOutdated",
                                        "sourceFile ts [" + sourceLastModified + "] differs from webinfClassFile ts [" + webinfLastModified + "]. Recompile JSP.");
                        }
                    }
                }
            } catch (IOException e) {
            ...
            if (outdated && generatedSourceFile.getParentFile().exists() == false) {
                ...
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
                    logger.logp(Level.FINEST, CLASS_NAME, "isOutdated",
                                (rc ? "Created" : "Unable to create") + " directory for generated source file [" + generatedSourceFile.getParentFile() + "]");
                }
            }
        } else {
            outdated = false;
    ...
    public static boolean isTagFileOutdated(long sourceLastModified, File generatedSourceFile, File classFile, Entry webinfEntry, File webinfClassFile) {
        ...
        if (generatedSourceFile.exists()) {
            if (sourceLastModified == generatedSourceFile.lastModified()) {
                ...
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                    ...
                    logger.logp(Level.FINER, CLASS_NAME, "isTagfileOutdated",
                                "sourceFile ts [" + sourceLastModified + "] differs from generatedSourceFile ts [" + generatedSourceFile.lastModified() + "]. Recompile tag file.");
                }
            }
            ...
            if (sourceLastModified == classFile.lastModified()) {
                ...
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                    ...
                    logger.logp(Level.FINER, CLASS_NAME, "isTagfileOutdated",
                                "sourceFile ts [" + sourceLastModified + "] differs from tempDirClassFile ts [" + classFile.lastModified() + "]. Recompile tag file.");
                }
            }
            ...
            if (sourceLastModified == webinfClassFile.lastModified()) {
                ...
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                    ...
                    logger.logp(Level.FINER, CLASS_NAME, "isTagfileOutdated",
                                "sourceFile ts [" + sourceLastModified + "] differs from webinfClassFile ts [" + webinfClassFile.lastModified() + "]. Recompile tag file.");
                }
            }
            ...
            if (sourceLastModified == webinfEntry.getLastModified()) {
                ...
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
                    ...
                    logger.logp(Level.FINER, CLASS_NAME, "isTagfileOutdated",
                                "sourceFile ts [" + sourceLastModified + "] differs from webinfEntry ts [" + webinfEntry.getLastModified() + "]. Recompile tag file.");
                }
            }
        }
        ...
        if (outdated && generatedSourceFile.getParentFile().exists() == false) {
            boolean rc = FileUtils.ensureDirExists(generatedSourceFile.getParentFile());
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
                logger.logp(Level.FINEST, CLASS_NAME, "isTagFileOutdated",
                            (rc ? "Created" : "Unable to create") + " directory for generated source file [" + generatedSourceFile.getParentFile() + "]");
            }
        }
    ...
    public static void syncGeneratedSource(long sourceFileTime, File generatedSourceFile) {
        if (generatedSourceFile.exists()) {
            ...
            if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
                logger.logp(Level.FINEST, CLASS_NAME, "syncGeneratedSource", (rc ? "Updated" : "Unable to update") + " lastModified timestamp for generatedSourceFile ["
                                                                             + generatedSourceFile + "] [" + generatedSourceFile.lastModified() + "]");
            }
        }
    ...
    public static void syncTagFile(long sourceFileTime, File generatedSourceFile, File classFile, boolean keepgenerated, boolean keepGeneratedclassfiles) {
        if (sourceFileTime == generatedSourceFile.lastModified()) {
            if (keepGeneratedclassfiles == false) {
                ...
                if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
                    logger.logp(Level.FINEST, CLASS_NAME, "syncTagFile",
                                (rc ? "Updated" : "Unable to update") + " lastModified timestamp for classFile [" + classFile + "] [" + classFile.lastModified() + "]");
                }
            }
            if (generatedSourceFile.exists()) {
                if (keepgenerated == false) {
                    ...
                    boolean rc = generatedSourceFile.setLastModified(sourceFileTime);
                    if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
                        logger.logp(Level.FINEST, CLASS_NAME, "syncTagFile", (rc ? "Updated" : "Unable to update") + " lastModified timestamp for generatedSourceFile ["
                                                                             + generatedSourceFile + "] [" + generatedSourceFile.lastModified() + "]");
                    }
                }
            }
