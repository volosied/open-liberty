...
public class JspConfiguration {
    ...
    protected JspConfiguration(JspConfigurationManager configManager) {
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
            logger.logp(Level.FINER, CLASS_NAME, "JspConfiguration",
                        "default values isXml = [{0}] isXmlSpecified = [{1}] elIgnored = [{2}] scriptingInvalid = [{3}] pageEncoding = [{4}] trimDirectiveWhitespacesValue = [{5}] deferredSyntaxAllowedAsLiteralValue = [{6}] trimDirectiveWhitespaces = [{7}] deferredSyntaxAllowedAsLiteral = [{8}] elIgnoredSetTrueInPropGrp = [{9}] elIgnoredSetTrueInPage = [{10}]",
                        new Object[] { Boolean.valueOf(isXml), Boolean.valueOf(isXmlSpecified), Boolean.valueOf(elIgnored), Boolean.valueOf(scriptingInvalid), pageEncoding,
                                       trimDirectiveWhitespacesValue, deferredSyntaxAllowedAsLiteralValue, trimDirectiveWhitespaces, deferredSyntaxAllowedAsLiteral,
                                       elIgnoredSetTrueInPropGrp, elIgnoredSetTrueInPage });
        }
    ...
    protected JspConfiguration(JspConfigurationManager configManager,
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINER)) {
            logger.logp(Level.FINER, CLASS_NAME, "JspConfiguration",
                        ...
                        new Object[] { Boolean.valueOf(isXml), Boolean.valueOf(isXmlSpecified), Boolean.valueOf(elIgnored), Boolean.valueOf(errorOnELNotFound),
                                       Boolean.valueOf(scriptingInvalid), pageEncoding, trimDirectiveWhitespacesValue, deferredSyntaxAllowedAsLiteralValue,
                                       trimDirectiveWhitespaces, deferredSyntaxAllowedAsLiteral, elIgnoredSetTrueInPropGrp, elIgnoredSetTrueInPage, errorOnELNotFoundInPropGroup,
                                       errorOnELNotFoundInPage });
        }
