...
public class JspConfigurationManager {
    ...
    public JspConfigurationManager(List jspPropertyGroups, boolean isServlet24, boolean isServlet24_or_higher, List addtlJspFileExtensions,
                                   boolean JCDIEnabled) throws JspCoreException {
    ...
    public synchronized JspConfiguration getConfigurationForUrl(String url) {
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
            logger.logp(Level.FINEST, CLASS_NAME, "getConfigurationForUrl", "begin creation of new JspConfiguration for url = [{0}] uris matched = [{1}]",
                        new Object[] { url, Integer.valueOf(matches.size()) });
        }
        JspConfiguration configuration = new JspConfiguration(this);
        if (matches.size() > 0) {
            if (isServlet24OrHigher)
                configuration.setElIgnored(false); // initial value may be overridden
            if (!isServlet25OrHigher)
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
            logger.logp(Level.FINEST, CLASS_NAME, "getConfigurationForUrl", "complete creation of new JspConfiguration for url = [{0}] {1}",
                        new Object[] { url, configuration.toString() });
        }
    ...
    public synchronized JspConfiguration getConfigurationForStaticInclude(String url, JspConfiguration parentConfig) {
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
            logger.logp(Level.FINEST, CLASS_NAME, "getConfigurationForStaticInclude",
                        "begin creation of new StaticIncludeJspConfiguration for url = [{0}] uris matched = [{1}] (config inherited from parent)",
                        new Object[] { url, Integer.valueOf(matches.size()) });
        }
        ...
        if (com.ibm.ejs.ras.TraceComponent.isAnyTracingEnabled() && logger.isLoggable(Level.FINEST)) {
            logger.logp(Level.FINEST, CLASS_NAME, "getConfigurationForStaticInclude", "complete creation of new StaticIncludeJspConfiguration for url = [{0}] {1}",
                        new Object[] { url, configuration.toString() });
        }
        return (configuration);
    }
